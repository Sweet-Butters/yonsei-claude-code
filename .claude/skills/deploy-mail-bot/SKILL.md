...
## 0. 어느 폴더에서 하는지부터 못 박는다 — 제일 먼저

**이 스킬의 모든 명령은 봇 폴더(`mail-notifier`) 안에서 실행한다.**
강의 저장소 루트에서 실행하면 **엉뚱한 저장소를 push 하게 된다.** 반드시 확인한다.

```
ls
```

`main.py` 와 `requirements.txt` 가 보이면 맞다. 안 보이면 봇 폴더로 이동한다.

```
cd mail-notifier
```

그래도 없으면 **여기서 멈추고** 3강을 먼저 하라고 안내한다. 절대 다음으로 넘어가지 않는다.
